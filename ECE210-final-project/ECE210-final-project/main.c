...
main(void)
{
  uint16_t 	x_pos = LCD_INIT_X + 50;		// x_pos holds the x position of the right corner of the image
  ...
  ece210_initialize_board();
	...
	ece210_lcd_add_msg("First to reach the end 3 times wins!", TERMINAL_ALIGN_CENTER, LCD_COLOR_GREEN);
  ...
  { 	
		...
		if (isCollidingWithLong(x_pos, y_pos, obs1_x, obs1_y) || isCollidingWith(x_pos, y_pos, obs2_x, obs2_y)
			...
			while(!done) {
				...
				ece210_lcd_add_msg(" ", TERMINAL_ALIGN_CENTER, LCD_COLOR_BLACK);
				ece210_lcd_add_msg(" ", TERMINAL_ALIGN_CENTER, LCD_COLOR_BLACK);
				ece210_lcd_add_msg("YOU LOST", TERMINAL_ALIGN_CENTER, LCD_COLOR_RED);
				done = true;
				}
			}
		}
		...
		if( AlertButtons)
		{
			...
			if( btn_right_pressed())
			{
				if((x_pos+width) < LCD_SIZE_X - MOVE_PIXELS)	// if not moving past the left edge
				{
					...
					ece210_wireless_send(RIGHT_BUTTON);					
				}				
			}
			if( btn_left_pressed())
			{
				if(x_pos > MOVE_PIXELS-1)														// if not moving past the right edge
				{
					...
					x_pos = x_pos - MOVE_PIXELS;
					ece210_wireless_send(LEFT_BUTTON);		
				}			
			}	
			...
			if( btn_down_pressed())
			{
				if((y_pos+height) < LCD_SIZE_Y-MOVE_PIXELS)	// if not moving past the top edge
				{
					...
					y_pos = y_pos + MOVE_PIXELS;
					ece210_wireless_send(DOWN_BUTTON);
				}			
			}
			...
			//Change opponent frogger position
				...
				if(ece210_wireless_data_avaiable())
				{
					...
					if( rx_data == DOWN_BUTTON) {
						if((y2_pos+height) < LCD_SIZE_Y-MOVE_PIXELS)														// if not moving past the bottom edge
						{
							ece210_lcd_draw_rectangle  (x2_pos, width, y2_pos, MOVE_PIXELS, LCD_COLOR_BLACK);
							y2_pos = y2_pos + MOVE_PIXELS;
						}
					}
					if( rx_data == LEFT_BUTTON) {
						if(x2_pos > MOVE_PIXELS-1)														// if not moving past the top edge
						{
							ece210_lcd_draw_rectangle  (x2_pos+width-MOVE_PIXELS, MOVE_PIXELS, y2_pos, height, LCD_COLOR_BLACK);
							x2_pos = x2_pos - MOVE_PIXELS;
						}
					}
					if( rx_data == RIGHT_BUTTON) {
						if((x2_pos+width) < LCD_SIZE_X - MOVE_PIXELS)														// if not moving past the bottom edge
						{
							ece210_lcd_draw_rectangle  (x2_pos, MOVE_PIXELS, y2_pos, height, LCD_COLOR_BLACK); 
							x2_pos = x2_pos + MOVE_PIXELS;		
						}
					}
				}
			...
			if (y_pos == 0) {
				...
				led_counter++;
				//User has reached the reached the end three times
				if(led_counter == 3) {
					while(1) {
						ece210_lcd_draw_image(x_pos,width, y_pos,height, p_image,LCD_COLOR_GREEN, LCD_COLOR_BLACK);
						while(!done) {
						...
						ece210_lcd_add_msg(" ", TERMINAL_ALIGN_CENTER, LCD_COLOR_BLACK);
						ece210_lcd_add_msg(" ", TERMINAL_ALIGN_CENTER, LCD_COLOR_BLACK);
						ece210_lcd_add_msg("YOU WON", TERMINAL_ALIGN_CENTER, LCD_COLOR_GREEN);
						ece210_lcd_add_msg(" ", TERMINAL_ALIGN_CENTER, LCD_COLOR_BLACK);
						ece210_lcd_add_msg(" ", TERMINAL_ALIGN_CENTER, LCD_COLOR_BLACK);
						ece210_lcd_add_msg(" ", TERMINAL_ALIGN_CENTER, LCD_COLOR_BLACK);
						ece210_lcd_add_msg(" ", TERMINAL_ALIGN_CENTER, LCD_COLOR_BLACK);
